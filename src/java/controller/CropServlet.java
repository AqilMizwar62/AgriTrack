...
public class CropServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action != null && action.equalsIgnoreCase("delete")) {
            handleDelete(request, response, userEmail);
        } else {
            try {
                CropDAO cropDao = new CropDAO();
                request.setAttribute("crops", cropDao.getCropsByUserEmail(userEmail));
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if ("add".equalsIgnoreCase(action)) {
                handleAddCrop(request, response, userEmail);
            } else if ("update".equalsIgnoreCase(action)) {
                handleUpdateCrop(request, response, userEmail);
            } else {
    ...
    private void handleDelete(HttpServletRequest request, HttpServletResponse response, String userEmail) 
            throws ServletException, IOException {
        try {
            int cropId = Integer.parseInt(request.getParameter("cropId"));
            CropDAO cropDao = new CropDAO();
            Crop cropToDelete = cropDao.getCropById(cropId, userEmail);
            ...
            if (cropDao.deleteCrop(cropId, userEmail)) {
                response.sendRedirect("crops");
            } else {
    ...
    private void handleAddCrop(HttpServletRequest request, HttpServletResponse response, String userEmail)
    ...
    private void handleUpdateCrop(HttpServletRequest request, HttpServletResponse response, String userEmail)
            throws ServletException, IOException, ParseException, SQLException {
        CropDAO cropDao = new CropDAO();
        Crop crop = new Crop();
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
